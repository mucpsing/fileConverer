...
set "srcript_path=\"W:\CPS\MyProject\projsect_persional\python-file-scripts\src\word2pdf\word_to_pdf_by_word.py\" \"%%1\"" 
set "fiexSuffix=.docx"
set "command=!command_path! !srcript_path!"

REM 添加任何目录的右键菜单中
reg add "HKCR\!fiexSuffix!\shell\%menu_name%" /ve /d "%menu_name%" /f
reg add "HKCR\!fiexSuffix!\shell\%menu_name%\command" /ve /d "!command!" /f
